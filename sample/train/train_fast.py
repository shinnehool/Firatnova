"""快速验证训练 — 100 张训练集，验证随机抽训练集测拟合能力。

用法:
    python -m train.train_fast                       # 使用 fast config
    python -m train.train_fast --resume              # 从最新 checkpoint 续训
"""
...
def train(config: dict, resume: bool = False):
    ...
    for epoch in range(start_epoch, total_epochs + 1):
        ...
        for batch_idx, (lr, lr_up, hr) in enumerate(pbar):
            ...
            # ---- Rectified Flow: 均匀采样 t ∈ [0,1]，覆盖完整 ODE 轨迹 ----
...
def validate(model: PixelDiTSR, val_loader, device: torch.device,
    ...
    for i, (lr, lr_up, hr) in enumerate(val_loader):
        ...
        # 单步直出：sr_single = lr_up - v_pred(lr_up, t=1)（诊断用）
        eval_model.eval()
        t_one = torch.ones(1, device=device)
        v_t1 = eval_model(lr_up, t_one, lr)
        sr_single = torch.clamp(lr_up - v_t1, 0.0, 1.0)

        # 核心指标：单步 PSNR/SSIM（模型在 t=1 的直接预测能力）
        psnr_val, ssim_val = batch_psnr_ssim(sr_single, hr)
        psnr_ode, ssim_ode = batch_psnr_ssim(sr, hr)
        ...
        if i == 0:
            ...
            save_triple(lr_up[0], hr[0], sr_single[0], "result/val_triple_single.png",
                        sr_second=sr[0])
            save_error_map(sr[0], hr[0], "result/val_error.png")
            save_spectrum_map(sr[0], hr[0], lr_up[0], "result/val_spectrum.png")
            logger.info(
                f"  ODE {config['inference']['num_ode_steps']}步: PSNR={psnr_ode:.2f} SSIM={ssim_ode:.4f} | "
                f"单步直出: PSNR={psnr_val:.2f} SSIM={ssim_val:.4f}"
            )
...
# ---------------------------------------------------------------------------
def main():
    parser = argparse.ArgumentParser(description="PixelDiT-SR 快速拟合验证")
    parser.add_argument("--config", type=str, default="config/config_fast.yaml")
